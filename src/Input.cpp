...
Input::Input(TTreeReader& r, bool hasGob, bool hasTN, bool hasQ, bool hasT) : reader(r) {

	// Set input switches only if constructor inputs are false
	if (!hasGob) hasGobbi = hasGob;
	if (!hasTN) hasTexNeut = hasTN;
	if (!hasQ) hasQDC = hasQ;
	if (!hasT) hasTDC = hasT;
	...
	vector<string> e_columns, eLo_columns, hinpt_columns, a_columns, b_columns, c_columns, psdt_columns, qh_columns, ql_columns, tdct_columns;
	if (hasGobbi) {
		e_columns     = GenerateColumnNamesHINP("e");
		eLo_columns   = GenerateColumnNamesHINP("eLo");
		hinpt_columns = GenerateColumnNamesHINP("t");
	}
	if (hasTexNeut) {
		a_columns     = GenerateColumnNamesPSD("a");
		b_columns     = GenerateColumnNamesPSD("b");
		c_columns     = GenerateColumnNamesPSD("c");
		psdt_columns  = GenerateColumnNamesPSD("t");
	}
	if (hasQDC) {
		qh_columns    = GenerateColumnNamesQDC("h");
		ql_columns    = GenerateColumnNamesQDC("l");
	}
	if (hasTDC) {
		tdct_columns  = GenerateColumnNamesTDC();
	}
	
	//// Create reader values for all columns, iteratively

	// HINP
	for (size_t i = 0; i < HINP_NCOLUMNS * hasGobbi; i++) {
	...
	// PSD
	for (size_t i = 0; i < PSD_NCOLUMNS * hasTexNeut; i++) {
